...
1. **Independent verification** that the Core physics implementation is correct
...
See `_background/equations.md` for complete documentation. Summary:
...
6. **edge_r_geq_b**: R=6, B=5 (dominated strategy, $R \geq B$)
...
- `src/Core/costs.jl` (withdrawal, resistance, dike costs)
- `src/Core/geometry.jl` (dike volume)
- `src/Core/zones.jl` (zone value calculations)
- `src/Core/types.jl` (parameter defaults)
...
1. Check `_background/equations.md` for the correct formula
2. Compare Core code to paper equations
3. Check if a new C++ bug was discovered
4. If new bug found, document it in `_background/equations.md` and fix in `icow_debugged.cpp`
