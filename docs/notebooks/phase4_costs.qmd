...
Dike cost is non-zero even at $D = 0$ due to startup costs ($D_{startup} = $(city.D_startup)$ m).
...
# Show startup cost
hline!([calculate_dike_cost(city, 0.0, 0.0) / 1e9],
       linestyle=:dash, color=:gray,
       label = "Startup cost (D = 0)", linewidth=1.5)

annotate!(D_range[end] * 0.5, calculate_dike_cost(city, 0.0, 0.0) / 1e9 * 1.2,
         text("Fixed startup cost\n(D_startup = $(city.D_startup) m)", 9, :gray))
```

**Physical interpretation:** Even a minimal dike incurs fixed costs (planning, permitting, base construction). The cost increases super-linearly due to the volume scaling with height squared.
