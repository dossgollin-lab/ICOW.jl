...
Dike cost is zero at $D = 0$ (not building a dike means no cost). When $D > 0$, costs increase due to the dike volume calculation which includes startup height ($D_{startup} = $(city.D_startup)$ m).
...
annotate!(D_range[end] * 0.6, maximum(C_D) / 1e9 * 0.3,
         text("D > 0: cost includes volume\nwith startup height offset", 9, :green))
```
...
- **No-build option**: $D = 0$ returns zero cost (no dike construction)
