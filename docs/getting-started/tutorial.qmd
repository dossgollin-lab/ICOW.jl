...
An `EADScenario` specifies the GEV parameters and a discount rate:

```{julia}
scenario = EADScenario(
    surge_loc=3.0,     # location (m): typical annual max surge
    surge_scale=1.0,   # scale (m): spread of surge heights
    surge_shape=0.0,   # shape: 0 = Gumbel (light-tailed)
...
The `simulate` function computes the total investment cost and expected flood damage over the planning horizon (50 years by default), discounted to present value:
...
scatter!(ax, inv_1, dmg_1; markersize=12, color=:steelblue, label="Gumbel (shape=0)")
